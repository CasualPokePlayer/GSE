...
internal sealed class RomLoader(Config config, EmuManager emuManager, PostProcessor postProcessor, OSDManager osdManager, GBController gbController, GBAController gbaController, ImGuiWindow mainWindow)
{
	...
	public static ImmutableArray<string> RomAndCompressionExtensions { get; } = [.._romExtensions, ..GSEFile.SupportedCompressionExtensions];

	public static ImmutableArray<string> BiosAndCompressionExtensions { get; } = [.._biosExtensions, ..GSEFile.SupportedCompressionExtensions];
	...
	private GSEFile ObtainBiosFile(bool gbaRom)
	{
		var biosPath = gbaRom
			? config.GbaBiosPath
	...
	private bool VerifyBiosFile(bool isGbaRom, ReadOnlyMemory<byte> biosData)
	{
		...
		var hash = Convert.ToHexString(GSEHash.HashDataSHA256(biosData.Span));
