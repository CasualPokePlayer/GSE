...
public sealed class GBAController(InputManager inputManager, EmuControllerBindings bindings, Func<InputGate> inputGateCallback) : IEmuController
{
	public EmuControllerState GetState(bool immediateUpdate)
	{
		...
		if (inputManager.GetInputForBindings(bindings.RButtonBindings, inputGate))
		{
			emuButtons |= EmuButtons.R;
		}

		if (inputManager.GetInputForBindings(bindings.LButtonBindings, inputGate))
		{
