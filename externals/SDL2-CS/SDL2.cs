...
// Many modifications have been done on this file, to better support the GSE's use cases
...
namespace SDL2
{
	public static partial class SDL
	{
		...
		internal static unsafe byte* Utf8Encode(string str, byte* buffer, int bufferSize)
		{
			...
			Encoding.UTF8.GetBytes(str, new(buffer, bufferSize - 1));
			buffer[bufferSize - 1] = 0;
			return buffer;
		}

		/* Used for heap allocated string marshaling.
		 * Returned byte* must be free'd with FreeCoTaskMem.
		 */
		internal static unsafe byte* Utf8EncodeHeap(string str)
		{
#if false
			// maybe this should be done rather?
			var bufferSize = Utf8Size(str);
			var buffer = (byte*)SDL_malloc(bufferSize);
			return Utf8Encode(str, buffer, bufferSize);
#else
			return (byte*)Marshal.StringToCoTaskMemUTF8(str);
#endif
		}
		...
		public static string UTF8_ToManaged(IntPtr s, bool freePtr = false)
		{
			...
			var result = Marshal.PtrToStringUTF8(s);
		...
		{
			public unsafe delegate* unmanaged[Cdecl]<IntPtr, long> size;
			public unsafe delegate* unmanaged[Cdecl]<IntPtr, long, int, long> seek;
			public unsafe delegate* unmanaged[Cdecl]<IntPtr, IntPtr, nuint, nuint, nuint> read;
			public unsafe delegate* unmanaged[Cdecl]<IntPtr, IntPtr, nuint, nuint, nuint> write;
			public unsafe delegate* unmanaged[Cdecl]<IntPtr, int> close;
		...
		public static unsafe IntPtr SDL_RWFromFile(
			...
			Marshal.FreeCoTaskMem((IntPtr) utf8Mode);
			Marshal.FreeCoTaskMem((IntPtr) utf8File);
			return rwOps;
		}
		...
		public static unsafe int SDL_ShowMessageBox(ref SDL_MessageBoxData messageboxdata, out int buttonid)
		{
			var data = new INTERNAL_SDL_MessageBoxData
			{
				...
				title = (IntPtr)Utf8EncodeHeap(messageboxdata.title),
				message = (IntPtr)Utf8EncodeHeap(messageboxdata.message),
			...
			for (var i = 0; i < messageboxdata.numbuttons; i++)
			{
				buttons[i] = new()
				{
					...
					text = (IntPtr)Utf8EncodeHeap(messageboxdata.buttons[i].text),
				};
			}

			if (messageboxdata.colorScheme.HasValue)
			{
				var colorSchemeSize = sizeof(SDL_MessageBoxColor) * (int)SDL_MessageBoxColorType.SDL_MESSAGEBOX_COLOR_MAX;
				data.colorScheme = Marshal.AllocCoTaskMem(colorSchemeSize);
				messageboxdata.colorScheme.Value.colors.AsSpan().CopyTo(new((void*)data.colorScheme, colorSchemeSize));
			}
			...
			Marshal.FreeCoTaskMem(data.colorScheme);
			for (var i = 0; i < messageboxdata.numbuttons; i++)
			{
				Marshal.FreeCoTaskMem(buttons[i].text);
			}
			Marshal.FreeCoTaskMem(data.message);
			Marshal.FreeCoTaskMem(data.title);
		...
		public static unsafe int SDL_GL_LoadLibrary(string path)
		{
			...
			Marshal.FreeCoTaskMem((IntPtr) utf8Path);
			return result;
		}
		...
		public static unsafe IntPtr SDL_CreateShapedWindow(string title, uint x, uint y, uint w, uint h, SDL_WindowFlags flags)
		{
			...
			Marshal.FreeCoTaskMem((IntPtr)utf8Title);
			return result;
		}
		...
		/* surface refers to an SDL_Surface* */
		public static unsafe bool SDL_MUSTLOCK(IntPtr surface)
		{
			var sur = (SDL_Surface*)surface;
		...
		public static unsafe int SDL_SetClipboardText(
			...
			Marshal.FreeCoTaskMem((IntPtr) utf8Text);
			return result;
		}
...
#pragma warning restore 0169
		...
		/* int refers to an SDL_JoystickType.
		 * Only available in 2.0.14 or higher.
		 */
		[LibraryImport(nativeLibName)]
		[UnmanagedCallConv(CallConvs = [ typeof(CallConvCdecl) ])]
		...
		public static unsafe int SDL_GameControllerAddMapping(
			...
			Marshal.FreeCoTaskMem((IntPtr) utf8MappingString);
			return result;
		}
		...
		public static SDL_GameControllerButtonBind SDL_GameControllerGetBindForAxis(
			...
			SDL_GameControllerButtonBind result = default;
		...
		public struct SDL_AudioSpec
		{
			...
			public ushort samples;
			public uint size;
			public unsafe delegate* unmanaged[Cdecl]<IntPtr, IntPtr, int, void> callback;
			public IntPtr userdata; // void*
		}
		...
		public static unsafe SDL_bool SDL_AndroidRequestPermission(
			...
			Marshal.FreeCoTaskMem((IntPtr) permissionPtr);
			return result;
		}
		...
		public static unsafe int SDL_AndroidShowToast(
			...
			Marshal.FreeCoTaskMem((IntPtr) messagePtr);
			return result;
		}
		...
		public struct INTERNAL_x11_wminfo
		{
			...
			public IntPtr window; // Refers to a Window (XID, use Tolong!)
		}
		...
		public struct INTERNAL_kmsdrm_wminfo
		{
			...
			public int drm_fd;
			public IntPtr gbm_dev; // Refers to a gbm_device*
		}
