...
GSE_EXPORT GSE_ctx* mgba_create(const uint8_t* romData, uint32_t romLength, const uint8_t* biosData, uint32_t biosLength, bool forceDisableRtc)
{
	set_default_logger();
...
GSE_EXPORT uint32_t mgba_getsavedatalength(GSE_ctx* ctx);

GSE_EXPORT void mgba_savesavedata(GSE_ctx* ctx, uint8_t* dest)
{
	ctx->sram_vf->seek(ctx->sram_vf, 0, SEEK_SET);
	ctx->sram_vf->read(ctx->sram_vf, dest, mgba_getsavedatalength(ctx));
}

GSE_EXPORT void mgba_loadsavedata(GSE_ctx* ctx, const uint8_t* data)
{
	ctx->sram_vf->seek(ctx->sram_vf, 0, SEEK_SET);
...
GSE_EXPORT bool mgba_loadstate(GSE_ctx* ctx, const uint8_t* stateBuf, uint32_t size)
{
