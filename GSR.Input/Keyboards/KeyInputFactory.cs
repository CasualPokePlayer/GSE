...
internal static class KeyInputFactory
{
	public static IKeyInput CreateKeyInput(in SDL_SysWMinfo mainWindowWmInfo)
	{
		...
		switch (mainWindowWmInfo.subsystem)
		{
			...
			case SDL_SYSWM_TYPE.SDL_SYSWM_X11 or SDL_SYSWM_TYPE.SDL_SYSWM_DIRECTFB when WlImports.HasDisplay && EvDevImports.HasEvDev && LibcImports.HasRoot:
				return new WlKeyInput(IntPtr.Zero);
			case SDL_SYSWM_TYPE.SDL_SYSWM_X11 or SDL_SYSWM_TYPE.SDL_SYSWM_DIRECTFB when X11Imports.HasDisplay:
				return new X11KeyInput();
			case SDL_SYSWM_TYPE.SDL_SYSWM_KMSDRM or SDL_SYSWM_TYPE.SDL_SYSWM_VIVANTE when EvDevImports.HasEvDev:
